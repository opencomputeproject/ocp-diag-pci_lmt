...
class TestPciLib(testslide.TestCase):
    def setUp(self) -> None:
        self.device = PciDevice("0000:11:2.3")
        super().setUp()

    def test_read_fails_invalid_args(self) -> None:
        """Tests if read method fails when invalid args are given."""
        # Invalid address.
        self.assertEqual(self.device.read(address=0xFFFF), -1)
        # Invalid width.
        self.assertEqual(self.device.read(address=0xFF, width=64), -1)
    ...
    def test_write_fails_invalid_args(self) -> None:
        ...
        # Invalid address.
        self.assertEqual(self.device.write(address=0xFFFF, data=0), -1)
        # Invalid width.
        self.assertEqual(self.device.write(address=0xFF, data=0, width=64), -1)
        # Unknown exception with valid address & val.
        with mock.patch("os.popen", side_effect=BaseException):
    ...
    def test_create_dict_capabilities(self) -> None:
        ...
        with mock.patch.object(self.device, "read", side_effect=[0x1000, 0x200A, 0x300B, 0]):
            self.device.create_dict_capabilities()
            self.assertEqual(list(self.device.cap_dict.keys()), [0xA, 0xB, 0])
    ...
    def test_create_dict_ext_capabilities(self) -> None:
        ...
        with mock.patch.object(self.device, "read", side_effect=[0x10A000, 0x20B000, 0]):
            self.device.create_dict_extended_capabilities()
            self.assertEqual(list(self.device.ext_cap_dict.keys()), [0xA000, 0xB000, 0])
    ...
    def test_get_link_status(self) -> None:
        ...
        with mock.patch.object(self.device, "read", side_effect=[0x1000, 0x2010, 0, 0x204]):
            self.assertEqual(
                self.device.get_link_status(),
                LinkStatusInfo(err_msg=None, speed=4, speed_gts="16GT/s", width=32),
            )
    ...
    def test_get_lmt_cap_info(self) -> None:
        ...
        with mock.patch.object(self.device, "read", side_effect=[0x0027, 0]):
            self.assertEqual(
                self.device.get_lmt_cap_info(),
                CapabilityInfo(err_msg=None, id=0x27, version=0, offset=0x100, offset_next=0),
            )
